...
const {
  result, filteredRows, displayColumns, searchQuery, selectedRows, hasSelection, isSingleSelection,
            ...
            <tbody>
              ...
              >
                <td v-for="col in displayColumns" :key="col">
                  <div v-if="col === 'Name'" class="name-cell" style="display:flex;align-items:center;gap:7px;overflow:hidden;">
                    <Table :size="13" class="text-accent flex-shrink-0" />
                    <span class="name-text">{{ row[col] }}</span>
                  </div>
                  <template v-else>{{ formatValue(col, row[col]) }}</template>
                </td>
              </tr>
            </tbody>
          </table>
